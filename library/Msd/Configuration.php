...
class Msd_Configuration
{
    ...
    {
        $this->_session = new Zend_Session_Namespace('oTranCe');
        $itemCount = $this->_session->getIterator()->count();
        if ($configName == '' && $itemCount != 0) {
            // get active configuration name from session
            $configName = $this->_session->dynamic->configFile;
        }
        ...
        $this->_data->dynamic =
            Msd_ConfigurationPhpValues::getDynamicValues();
