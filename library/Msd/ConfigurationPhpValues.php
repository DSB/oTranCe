...
class Msd_ConfigurationPhpValues
{
    ...
    /**
     * Detect if zlib is installed and can be used to compress files
     *
     * @return boolean
     */
    ...
    {
        $zlib = false;
        $disabledFunctions = self::_getConfigSetting('disable_functions');
        $disabledFunctions = str_replace(',', ', ', $disabledFunctions);
        $extensions = get_loaded_extensions();
        $disabledExtensions = str_replace(' ', '', $disabledFunctions);
        $disabledExtensions = explode(',', $disabledExtensions);
        if (in_array('zlib', $extensions)) {
            $zlib = true;
        };
        if (in_array('gzopen', $disabledExtensions)
     ...
     * @param boolean $returnAsInt Wether to return value as integer
     */
